...
pub struct TinyT5Polisher {
    inner: T5OnnxGec,
}

impl TinyT5Polisher {
    pub fn load_from_dir(dir: &Path) -> Result<Self, GrammarError> {
        if !model_is_complete(GrammarModelId::Tiny, dir) {
            return Err(GrammarError::Other(format!(
                "tiny GEC model files missing in {}",
                dir.display()
            )));
        }
        let inner = T5OnnxGec::load_from_dir(dir, "")?;
        Ok(Self { inner })
    }
}

impl GrammarPolisher for TinyT5Polisher {
    fn is_loaded(&self) -> bool {
        true
    }

    fn model_id(&self) -> Option<GrammarModelId> {
        Some(GrammarModelId::Tiny)
    }
